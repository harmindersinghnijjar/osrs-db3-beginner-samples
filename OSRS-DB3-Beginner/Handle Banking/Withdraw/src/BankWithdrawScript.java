import org.dreambot.api.methods.container.impl.bank.Bank;
import org.dreambot.api.methods.interactive.Players;
import org.dreambot.api.script.AbstractScript;
import org.dreambot.api.script.Category;
import org.dreambot.api.script.ScriptManifest;
import org.dreambot.api.utilities.Sleep;

@ScriptManifest(name = "Withdrawing Items", description = "Open the nearest bank and withdraw items.", author = "Harminder Singh Nijjar",  version = 0.1,
        category = Category.MISC, image = "")
public class BankWithdrawScript extends AbstractScript {

    @Override
    public int onLoop() {
        if (Bank.open()) {
            // If this returns true, that means the bank is open and ready
            Bank.withdraw(995);         // Both of these will withdraw one coin
            Bank.withdraw("Coins");

            Bank.withdraw(995, 10);     // Both of these will withdraw 10 coins
            Bank.withdraw("Coins", 10);

            Bank.withdrawAll(995);      // Both of these will withdraw all coins
            Bank.withdrawAll("Coins");

            // Once you are done withdrawing, you can close the bank
            Bank.close();

            //Log that we are done depositing
            log("Done withdrawing");
            // Sleep while the character is standing still or 5 minutes has passed
            Sleep.sleepWhile(() -> !Players.getLocal().isMoving(), 300000);

        } else {
            // If this returns false, that means the client is still walking
            // or interacting with the bank
            return 500;
        }

        return 500;
    }

}

